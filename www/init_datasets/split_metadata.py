...
def split_and_save_data(vm, target_key_type, method="assign", no_link=False):
    ...
    # 指定划分
        ...
        # 按照时间划分
            ...
            # [::-1]相当于[-1:-len(target_keys)-1:-1]，也就是从最后一个元素到第一个元素,倒序
...
def print_distribution(vm, target_key_type):
    ...
    # 按照target_key_type分类后，每种类别的正样本、负样本、总样本、正样本率
...
def main(argv):
    ...
    split_and_save_data(vm, "date", method="random", no_link=no_link)
    split_and_save_data(vm, "camera", method="random", no_link=no_link)
